...
CREATE OR REPLACE VIEW v_census_current AS
SELECT
    l.facility_name,
    l.unit,
    l.location_type,
...
FROM metric_patient_census c
JOIN dim_location l ON c.facility = l.facility_name AND c.unit = l.unit
WHERE c.census_date = CURRENT_DATE;

-- Census Trend (Last 30 Days)
CREATE OR REPLACE VIEW v_census_trend AS
SELECT
    census_date,
    facility,
...
CREATE OR REPLACE VIEW v_census_by_day_of_week AS
SELECT
...
CREATE OR REPLACE VIEW v_inpatient_by_unit AS
SELECT
    l.unit,
    l.location_type,
    l.service_line,
...
JOIN dim_location l ON fe.location_key = l.location_key
